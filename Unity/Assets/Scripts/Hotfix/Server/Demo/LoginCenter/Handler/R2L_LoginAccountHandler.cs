namespace ET.Server
{
    ...
    public class R2L_LoginAccountHandler: MessageHandler<Scene, R2L_LoginAccountRequest, L2R_LoginAccountResponse>
    {
        protected override async ETTask Run(Scene scene, R2L_LoginAccountRequest request, L2R_LoginAccountResponse response)
        {
            ...
            using (await coroutineLockComponent.Wait(CoroutineLockType.LoginCenterLock, accountId.GetHashCode()))
            {
                ...
                StartSceneConfig gattConfig = RealmGateAddressHelper.GetGate(zone, request.AccountName);
                ...
                G2L_DisconnectGateUnit g2LDisconnectGateUnit =
                        await scene.GetComponent<MessageSender>().Call(gattConfig.ActorId, l2GDisconnectGateUnit) as G2L_DisconnectGateUnit;
